...
def get(html):
    ...
    #     "effect": "1",
    #     "is_special_job": "",
    #     "job_href": "https://jobs.51job.com/shanghai/124524237.html?s=01&t=0",
    #     "job_name": "临床产品经理",
    #     "job_title": "临床产品经理",
    #     "company_href": "https://jobs.51job.com/all/co2281874.html",
    #     "company_name": "北京美科创新科技有限公司",
    ...
    #     "degreefrom": "6",
    #     "workyear": "5",
    #     "issuedate": "2020-08-19 08:58:17",
    #     "isFromXyz": "",
    #     "jobwelf": "五险一金 交通补贴 通讯补贴 绩效奖金 年终奖金 定期体检",
    #     "jobwelf_list": [
    #         "五险一金",
    #         "交通补贴",
    #         "通讯补贴",
    ...
    # reg = re.compile(
    #     r'class="t1 ">.*? <a target="_blank" title="(.*?)" href="(.*?)".*? <span class="t2"><a target="_blank" title="(.*?)".*?<span class="t3">(.*?)</span>.*?<span class="t4">(.*?)</span>.*? <span class="t5">(.*?)</span>',
    #     re.S)  # 匹配换行符
    # items = re.findall(reg, html)
    return items
...
def get_job_desc(url):  # 获取职位描述
    ...
    # 职能类别
    reg = re.compile(
        r'<div class="mt10">.*?<p class="fp">.*?<span class="label">职能类别：</span>(.*?)</p>.*?</div>.*?<div class="share">',
        re.S)
    temp = re.findall(reg, html)[0]
    items.append(temp.replace("\r", "").replace(
...
def thread_process(startPage, endPagae, jobName):
    ...
    headData = ['JobName', 'JobURL', 'Company', 'Adress', 'Salary', 'Date', 'AllJobUrl', 'CompanyType',
                'CompanySize', 'Industry', 'Education', 'Experience', 'Number', 'Welfare', 'JobDesc', 'JobLabel',
                'ContactAdress']  # 表头部信息
